...
//  sphinx
//
//  Lightweight GFM-compatible markdown renderer producing NSAttributedString.
//  Covers: headings, bold, italic, bold-italic, strikethrough, inline code,
//  fenced code blocks, unordered/ordered lists, task lists, blockquotes, links.
//
...
struct MarkdownStyle {
    ...
    var baseFont:    UIFont { UIFont.systemFont(ofSize: baseFontSize) }
    var boldFont:    UIFont { UIFont.boldSystemFont(ofSize: baseFontSize) }
    var italicFont:  UIFont { UIFont.italicSystemFont(ofSize: baseFontSize) }
    var boldItalicFont: UIFont {
        let descriptor = UIFont.systemFont(ofSize: baseFontSize).fontDescriptor
            .withSymbolicTraits([.traitBold, .traitItalic]) ?? UIFont.systemFont(ofSize: baseFontSize).fontDescriptor
        return UIFont(descriptor: descriptor, size: baseFontSize)
    }
    var codeFont:    UIFont { UIFont(name: "Menlo-Regular", size: codeFontSize) ?? UIFont.monospacedSystemFont(ofSize: codeFontSize, weight: .regular) }

    func headingFont(level: Int) -> UIFont {
        let sizes: [CGFloat] = [22, 20, 18, 16, 15, 15]
...
final class MarkdownRenderer {
    ...
    func renderInline(_ text: String, font: UIFont, color: UIColor) -> NSAttributedString {
        let result = NSMutableAttributedString()
        var s = text
        // Base attrs
        ...
        while !s.isEmpty {
            ...
            // sphinx.chat:// deep link (any action)
