...
class ChatMentionAutocompleteDataSource : NSObject {
    ...
    func updateMentionSuggestions(suggestions: [(String, String)]){
        DispatchQueue.main.async {
            ...
            self.mentionSuggestions = suggestionObjects ?? []
            ...
            if (suggestions.isEmpty == false) {
                ...
                self.tableView.scrollToRow(at: bottom, at: .bottom, animated: true)
            }
        }
    }
    ...
    func updateMacroSuggestions(macros: [MentionOrMacroItem]) {
        DispatchQueue.main.async {
            self.tableView.isHidden = (macros.isEmpty == true)
            self.mentionSuggestions = macros
