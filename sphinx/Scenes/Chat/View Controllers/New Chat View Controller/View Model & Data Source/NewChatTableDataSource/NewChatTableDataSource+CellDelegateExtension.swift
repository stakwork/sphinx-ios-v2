    ...
    func shouldLoadImageDataFor(
        ...
        if var tableCellState = getTableCellStateFor(
            ...
            if message.isDirectPayment() {
                MediaLoader.loadPublicImage(url: imageUrl, messageId: message.id, completion: { [weak self] messageId, image in
                ...
                // Use MediaPreloadManager to ensure downloads continue even when view is deallocated
                MediaPreloadManager.sharedInstance.loadImage(
                    url: imageUrl,
                    message: message,
                    mediaKey: mediaKey,
                    completion: { [weak self] messageId, image in
                        let updatedMediaData = MessageTableCellState.MediaData(
                            image: image
                        )
                        self?.updateMessageTableCellStateFor(rowIndex: rowIndex, messageId: messageId, with: updatedMediaData)
