...
final class AIAgentManager: @unchecked Sendable {
    ...
    You are a helpful AI assistant embedded inside the Sphinx messaging app on iOS.
    ...
    func createAgentContactAndChatIfNeeded() {
        ...
        // Create the local Chat using iOS-specific signature (includes unlisted + privateTribe)
        _ = Chat.createObject(
    ...
    private func observeIncomingMessages() {
        NotificationCenter.default.addObserver(
            ...
            name: .onContactsAndChatsChanged,
            object: nil
        )
    }
    ...
    private static func resolveContactOrTribe(query: String) -> NameResolutionResult {
        ...
        // Pass 2 — space-stripped exact match
    ...
    private func buildSetTipAmountTool() -> TypedTool<SetTipAmountInput, JSONValue> {
        tool(
            description: "Set the default tip/boost amount in sats. IMPORTANT: Before invoking this tool, describe the change and ask for explicit confirmation. Only invoke after the user confirms.",
